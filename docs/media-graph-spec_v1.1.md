...
> **Upheld (Spielplan, 2026-09-17, decision 291).** `spielplan-spec_v2.1.md`'s §4.1, §4.3 and §10 reversed this sentence without a note and shipped the Tag Genome as a 983-column feature block of 888,023 `ml_genome_score` rows. The import stops. **Corrected at M4.16 review cycle 1 (decision 304):** the ruling stands and the reason it was first given for does not. The block fed nothing for the titles §8 acquires, which carry no genome vector at all — but on v20260828 a fifth of the 5,315 titles that do carry one are not warm, which is the set the Cold Tower is asked to place, and nearly all of those carry a cold-masked Backbone row that `spielplan-spec_v2.1.md` §5.1 already treats as no coordinate. For them the block was a live input worth about a third of their non-zero mass, and the loss is accepted rather than denied. The three MovieLens tables travel in the bundle and are loaded by no import this app performs, so the feature contract's genome columns are zero for every title this build seeds; an install seeded before decision 291 keeps the rows it already loaded and still reads them, which is the one case that sentence does not cover and is not a leak (decision 311). The restriction above stands exactly as written.
