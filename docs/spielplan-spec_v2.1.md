...
- Per-user table/board of every rated title in tiers **F, D, C, B, A, A+, S** (configurable set; learned cutpoints, not percentile cuts — initialised from DNA_MODEL §4.5's measured quantile shape F 3 / D 7 / C 15 / B 25 / A 25 / A+ 17 / S 8 %, then learned). Badge shows tier + neighbourhood ("A — between Heat and Prisoners"); a title whose posterior reaches an adjacent tier within `straddle_z`·σ shows "A/S" and becomes queue-eligible. `straddle_z` is a **badge** constant and must be tuned against the σ the fit actually produces: on a real household board the posterior sd (≈1.2) exceeds a tier's width (≈1.1), so z = 1.0 badges ~99% of the board, leaves the 70% boundary arm and the 20% exploration arm drawing from the same set, and makes the chip carry no information. The shipped value is 0.15. Any definition stated as posterior mass is the same predicate re-parameterised and inherits the same problem — at that σ the mass outside a title's own tier is at least 0.65 for every title, so a threshold below that can never exclude anything (decisions 175 and 214).
...
| **M4** | Tonight: lobby + open-rooms discovery, push join, the ~10-vote round, guest hand-off, group combine + conflict surfacing, blind reveal, **solo mode** | a real Friday night resolved by the app |
